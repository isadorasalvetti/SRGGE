...
bool ReadPlyHeader(std::ifstream *fin, int *vertices, int *faces) {
  char line[100]; //name of the file

  //ifstream fin is the [object] file to open
  fin->getline(line, 100); //stores the chosen file name to line 
  //compares characters on line to "ply" = is this a ply file?
  
  if (strncmp(line, "ply", 3) != 0) return false; //if not, stop here [return].

  *vertices = 0; //set [pointed] vertices to 0.
...
bool ReadFromPly(const std::string &filename, TriangleMesh *mesh) {
  ...
  ComputeVertexNormals(mesh->vertices_, mesh->faces_, &mesh->normals_, &mesh->faceNormals_);
